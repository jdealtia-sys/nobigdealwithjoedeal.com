...
// Transition reconciliation: every paymentsOf copy must append a synthetic
...
// Copies live in money-dashboard, analytics-kpi, and pages/leaderboard.
const lb = read('docs/pro/js/pages/leaderboard.js');
ok('money + analytics + leaderboard paymentsOf reconcile ledger shortfall (synthetic remainder)',
  /remainderCents/.test(md) && /synthetic: true/.test(md)
  && /remainderCents/.test(ak) && /synthetic: true/.test(ak)
  && /remainderCents/.test(lb) && /synthetic: true/.test(lb));
