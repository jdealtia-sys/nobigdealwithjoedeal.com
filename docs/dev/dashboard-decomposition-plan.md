...
| 2c-4+ | remaining allowlist clusters (dashboard-actions.js, dashboard-bootstrap.module.js 18, dashboard-ui.js 16, …) — same registry pattern, one module per PR | 🔄 in progress — **dashboard-actions.js audited 2026-07-07** (`dashboard-actions-globals-audit.md`): the "87" conflated owned vs forwarded globals; it owns ~67, of which ~46 CONVERTIBLE / 14 MUST-STAY (incl. the `maps.js:464-468` unguarded zone-draw shims + the `goTo` router). Ordered slices `2c-4a…2c-4e` + a shim-blocked residual. **2c-4a ✅** (card-detail `cda*`, 18 — full E2E green), **2c-4b ✅** (mobile create/job-detail — 3 registered, 7 window re-exports, 1 private), and **2c-4c ✅** (20 one-off openers, all REGISTER_ONLY, incl. the `mCreateFabRoute`/`mQuickAddRoute` tail) SHIPPED 2026-07-07. Next: `2c-4d` (daily-program `ds*`, 6) then `2c-4e` (customer-page handoff, 4). Then the shim-blocked residual (zone-draw, `damagNearMe`, `goTo` router). |
