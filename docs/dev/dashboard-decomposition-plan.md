...
| 2c-1 | `__NBD_CALL_REGISTRY` dispatch layer + prefs-boot cluster (22 globals off window) | ✅ merged 2026-07-06 — all three dashboard-ui.js dispatchers (`call` click, change/input, `data-on-after`) now resolve `window.__NBD_CALL_REGISTRY` FIRST via `_nbdResolveCall`, then fall back to allowlisted window globals. Converted modules REGISTER their handlers instead of exposing them (registration = the security opt-in; markup unchanged, so dashboard.legacy.html keeps working off the shared live JS). dashboard-ui-prefs-boot.js is the first converted module: IIFE-wrapped, its 22 delegate-called names registered + removed from the allowlist; only `showToast`/`nbdSyncSizeBtns`/`nbdRenderFontGrid` stay on window (real cross-file window consumers). NEW CI invariant: the smoke wiring audit asserts every `data-fn`/`data-on-change`/`data-on-input`/`data-on-after` name in dashboard.html is allowlisted-or-registered — the C-1 saveLead / H-4 dead-button class is now caught structurally. Inventory basis for the remaining clusters: 242 allowlisted names by defining file — dashboard-actions.js 87, maps-routing.js 22, prefs-boot 22 (done), dashboard-bootstrap.module.js 18, dashboard-ui.js 16, crm-portal-bridge.js 12, remainder ≤7 each |
| 2c-2 | maps-routing.js cluster: whole 3,4xx-line drawing-tool file IIFE-wrapped; 21 of its 22 allowlisted names registered in `__NBD_CALL_REGISTRY` + removed from the allowlist + pinned off window | ✅ 2026-07-06 — converted: acceptAutoDetect, addStructure, applySmartWaste, cancelAutoDetect, exportXactimateESX, generateScopeFromDrawing, handleComparisonFile, loadDrawingFromCustomer, openComparisonMode, recalc, runSolarAnalysis, saveDrawingToCustomer, screenshotMap, setHistoricalLayer, showAngles, showMaterialTakeoff, startAutoDetect, startPresentation, startShadowPitch, updateHistoryOpacity, zoomToFit. **goToMyLocation FAILED the three-way proof** (maps.js shim line `window.goToMyLocation = goToMyLocation` is a live cross-file code ref) → stays on window + allowlisted; Tranche 3 with the shim's export block. Wrap constraints found: `drawMap` must stay a top-level `let` OUTSIDE the IIFE (bare-read at runtime by dashboard-actions.js ensureMapSize, dashboard-ui.js spyglass jump, dashboard-sw-bootstrap.js diagnostics); 16 more names need explicit window exports because they dispatch as `window[fn]` OUTSIDE the registry-first path — `_NBD_TOGGLE_FNS` (toggleDraw/toggleMapLayer/toggleHistoricalImagery/toggleVoiceControl) and `_NBD_MODAL_CLOSE_FNS` (closeComparisonMode/closeHistoricalImagery) resolve window-only, initDrawMap is polled via `window[fnName]` (waitForMapFn), selLT + renderAccessoryPanel + setDrawMode are bare-called cross-file, and setDrawMode/clearDraw/undoLine/exportDrawReport/importToEstimate/perimChooseType/searchDraw are allowlisted data-fn names belonging to the dashboard-actions.js cluster. Bonus surface shrink: deleteLine/selectLine/deselectLine/retypeLine/erToggleSegment dropped off window — their only external refs are dashboard-actions.js's typeof-guarded re-export block, which is dead code for maps names (dashboard-actions executes BEFORE maps-routing in the defer queue, so the guards always skipped) |
| 2c-3 | crm-portal-bridge.js cluster: whole 741-line file IIFE-wrapped; 11 of its 12 allowlisted names registered in `__NBD_CALL_REGISTRY` + removed from the allowlist + pinned off window | ✅ 2026-07-07 (PR #898) — converted: selectAllVisibleLeads, openDeletedDrawer, confirmDeleteLead, cancelDeleteConfirm, bulkSnoozeLeads, bulkMoveStage, bulkDelete, bulkAssignSource, bulkAssignJobType, bulkAssignDamage, bulkAssignCarrier. **clearBulkSelection FAILED the three-way proof** (lead-snooze.js:773 calls `window.clearBulkSelection()` OUTSIDE the registry-first dispatcher) → stays on window + allowlisted, same MUST-STAY shape as goToMyLocation. Wrap surface: 10 more names need explicit window re-exports because they resolve as bare cross-file globals / `window.X()` / `window[fn]` dispatch — editLead (crm-pipeline.js:351 bare + 4 more), deleteLead (kanban-context-menu.js:273), **showDeleteConfirm (maps-overlays.js:358 bare — the sleeper: never in crm.js's re-export list, survived only as a classic auto-global)**, toggleBulkMode + closeDeletedDrawer (`_NBD_TOGGLE_FNS`/`_NBD_MODAL_CLOSE_FNS` window[fn]), toggleCardSelection (crm-pipeline.js:2007), updateBulkToolbar, scrollToFollowUps (analytics-kpi.js:841), restoreCrmSearch + refreshTrashBadge (dashboard-bootstrap.module.js). crm.js's 21 bare re-export lines for these names were deleted (they'd ReferenceError once module-scoped); the always-dead delete/drawer typeof-guards in dashboard-actions.js were removed. |
| 2c-4+ | remaining allowlist clusters (dashboard-actions.js, dashboard-bootstrap.module.js 18, dashboard-ui.js 16, …) — same registry pattern, one module per PR | ✅ **COMPLETE 2026-07-08 (status verified against git 2026-07-14):** — **dashboard-actions.js audited 2026-07-07** (`dashboard-actions-globals-audit.md`): the "87" conflated owned vs forwarded globals; it owns ~67, of which ~46 CONVERTIBLE / 14 MUST-STAY (incl. the `maps.js:464-468` unguarded zone-draw shims + the `goTo` router). Ordered slices `2c-4a…2c-4e` + a shim-blocked residual. **dashboard-actions.js cluster COMPLETE** — 2c-4a…2c-4e all SHIPPED 2026-07-07 (~57 names off window across 5 in-file IIFEs): **2c-4a** card-detail `cda*` (18, full E2E green), **2c-4b** mobile create/job-detail (3 reg + 7 window + 1 private), **2c-4c** one-off openers (20, incl. `mCreateFabRoute`/`mQuickAddRoute` tail), **2c-4d** daily-program `ds*` (3 reg + 1 window `dsRemoveFloor` + 3 private; typeof block deleted), **2c-4e** customer handoff (4 reg + `_stashLeadForCustomerPage` window). Only the **shim-blocked residual** remains (zone-draw — needs the `maps.js:464-468` unguarded shims unwound; `damagNearMe` — dedup vs `maps-overlays.js`; the `goTo` router — never converts). **Both follow-on module clusters shipped too:** 2c-4f (#901) took the `dashboard-bootstrap.module.js` settings/debug/export cluster off window (15 registered; MUST-STAYs per the allowlist notes), and 2c-4g (#902) + 2c-4h Slices H1/H2 (#903–#905) finished `dashboard-ui.js` (leaf handlers, the 9 clean-needs-wrap names, and the 10 entangled twins/shims incl. the property-intel dedup). Remaining in the registry lane: only the shim-blocked residual above — then Tranche 3. **UPDATE 2026-08-31: the registry lane is CLOSED.** The residual's two stated blockers had already closed 2026-08-07 (PR #1194 guarded the `maps.js` re-export block and deduped `damagNearMe`); the deferred conversion itself shipped 2026-08-31 as **Tranche 3 slice T3-0** — `selectZoneColor`/`startZoneDraw`/`cancelZoneDraw`/`saveZone`/`deleteZone`/`damageNearMePhotos` IIFE-scoped + registered, the six `maps.js` re-exports deleted, two cross-file bare calls rewired (`dashboard-widgets.js` zone-delete → `data-action`, `dashboard-ui.js` `zoneColor` → `_nbdResolveCall`). `renderSavedZones` MUST-STAY; `goTo` never converts. The `maps.js:464-468` reference in this row has pointed at comment text since 2026-08-07 — don't chase it. |
