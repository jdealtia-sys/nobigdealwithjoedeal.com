...
| 2c-4+ | remaining allowlist clusters (dashboard-actions.js, dashboard-bootstrap.module.js 18, dashboard-ui.js 16, …) — same registry pattern, one module per PR | ⏳ next — **dashboard-actions.js audited 2026-07-07**: the "87" conflated owned vs forwarded globals; it owns ~67, of which ~46 CONVERTIBLE / 14 MUST-STAY (incl. the `maps.js:464-468` unguarded zone-draw shims + the `goTo` router). Broken into ordered one-PR slices `2c-4a…2c-4e` + a shim-blocked residual — see `dashboard-actions-globals-audit.md`. Do `2c-4a` (card-detail `cda*`, 18 names) first. |
