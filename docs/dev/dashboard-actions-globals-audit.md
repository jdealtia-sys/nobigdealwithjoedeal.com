...
|---|---|---|---|---|
| **2c-4a** | Card-detail (`cda*`) + chip pickers + `_mCreatePhotoPicked` | 18 | LOW | **Do first.** Cohesive, all delegate to other modules (`NBDReports`, `PhotoEngine`) or `window._cardDetailLeadId`; no shared top-level lexical state. Route `cdaMjdAct`→`window._mJdAct`, `cdaOpenMobileInspection`→`window.openMobileInspection`. Migrate 14 guardrail entries. |
| **2c-4b** | Mobile create/job-detail | 8 | MED | MUST-STAYs in this region (`closeMobileInspection`, `closeMobileCreatePopover`, `openLeadDetail`, `_mJdSwitchTab`, `viewProspectOnMap`) get `window.` re-exports from the IIFE. Reconcile the 2c-4a→callee cross-IIFE calls here. |
| **2c-4c** | One-off compound rewrites + openers | 18 | LOW | Mostly self-contained delegators created to replace inline `onclick` compounds; several have no live markup (dead-but-safe). |
| **2c-4d** | Daily-program config (`ds*`) | 6 | LOW | Convert `dsAddFloor`/`dsSaveConfig`/`dsResetDefaults` + 3 internal helpers; `dsRemoveFloor` MUST-STAY (re-export). |
| **2c-4e** | Customer-page handoff | 4 | LOW | `openPhotosForLead`, `openDocsForLead`, `openFullCustomerDetails`, `editCardDetails`; `_stashLeadForCustomerPage` MUST-STAY (re-export). |
