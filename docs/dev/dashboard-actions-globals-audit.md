...
| ~~`closeMobileInspection`~~ ✅ | `_NBD_MODAL_CLOSE_FNS` (key `mobileInspection`) → `window[fnName]` | ~~dynamic dispatch~~ **CONVERTED 2026-09-01** — registry-only. The map resolves `__NBD_CALL_REGISTRY` first now (Tranche 3 slice T3-M), so the dispatch shape stopped being a blocker. |
| ~~`closeMobileCreatePopover`~~ ✅ | `_NBD_MODAL_CLOSE_FNS` (key `mobileCreatePopover`) | ~~dynamic dispatch~~ **CONVERTED 2026-09-01** — registry-only, same reason. |
