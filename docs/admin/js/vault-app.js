...
function renderOverview() {
  ...
  const recentHTML = CODEX.sessions
    ...
    .reverse()
    .map(s => `
      <div style="margin-bottom:10px; padding:10px; background:var(--lift); border-radius:var(--r); cursor:pointer" data-action="view-session" data-num="${s.num}">
        <strong style="color:var(--text)">Session ${s.num}</strong> — ${esc(s.meta.date || 'No date')}<br>
        <span style="font-size:13px; color:var(--text3)">${esc(s.meta.title || 'Untitled')}</span>
      </div>
    `).join('');
...
// Defense-in-depth HTML escaper. The Vault is Jo's UID-locked personal
// build-log (firestore.rules pins codex/* to his uid), so these fields aren't
// attacker-reachable — but escaping keeps the innerHTML sinks honest and
// covers the AI-generated insights (Claude output steered by pasted notes).
function esc(s) {
...
function renderSessions() {
  ...
  const sessionsHTML = CODEX.sessions
    ...
    .map(s => `
      <div class="session-card" data-action="view-session-detail" data-num="${s.num}">
    ...
    <div id="sessionDetail" style="display:none; margin-top:24px;">
      <div class="card">
        <div class="card-header">
          ...
          <button class="btn-outline btn-sm" data-action="close-session-detail">Close</button>
        </div>
    ...
    html += `
      <div style="margin-bottom:24px;">
        ...
        ${results.sessions.map(s => `
          <div class="card" style="margin-bottom:12px; cursor:pointer;" data-action="view-session" data-num="${s.num}">
            <div class="card-header">
              <div style="flex:1;">
  ...
  page.innerHTML = `
    ...
    <div class="card" id="sessionStarterSection">
      <div class="card-header">
        ...
        <button class="btn-gold btn-sm" data-action="generate-session-starter">Generate Prompt</button>
      </div>
      <div class="card-body">
        ...
        <button class="btn-outline btn-sm" style="margin-top:12px" data-action="copy-session-starter">📋 Copy to Clipboard</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <button class="btn-gold btn-sm" style="margin-top:12px" data-action="import-session-payload">Import to Firestore</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <button class="btn-gold btn-sm" data-action="manual-save-firestore">💾 Save to Firestore Now</button>
      ...
      <div class="card-body">
        ...
        <div style="background:var(--lift); border:1px solid var(--border); border-radius:var(--r); padding:16px; margin-bottom:16px;">
          ...
          <div style="display:flex; gap:8px; margin-bottom:12px;">
            ...
            >
            <button class="btn-outline btn-sm" data-action="parse-session-test">🔍 Test Parse</button>
          </div>
        ...
        <div style="background:var(--gold-dim); border:1px solid var(--gold); border-radius:var(--r); padding:16px; margin-bottom:16px;">
          ...
          <button class="btn-gold btn-sm" id="parseAllBtn" data-action="parse-all-sessions">🔄 Parse All Sessions</button>
        </div>
...
function importSessionPayload() {
  ...
  try {
    // CSP-safe payload execution: the production CSP has no 'unsafe-eval',
    // so eval() is blocked. The payload format is vaultImportSession({...})
    // or vaultBulkImportSessions([...]) with a JSON-serializable argument —
    // parse the argument and dispatch to the named import function.
    const match = payload.match(/^\s*(vaultImportSession|vaultBulkImportSessions)\s*\(([\s\S]*)\)\s*;?\s*$/);
    if (!match) throw new Error('Payload must be vaultImportSession({...}) or vaultBulkImportSessions([...])');
    window[match[1]](JSON.parse(match[2]));
  ...
  modal.innerHTML = `
    ...
    ">
      ...
      <div style="margin-bottom: 24px;">
        ...
        <button class="btn-gold" data-action="duplicate-action" data-choice="rename" data-num="${suggestedNum}" style="width: 100%; margin-bottom: 12px; justify-content: flex-start;">
        ...
        <button class="btn-outline" data-action="duplicate-action" data-choice="version" style="width: 100%; margin-bottom: 12px; justify-content: flex-start;">
          <span style="flex: 1; text-align: left;">
        ...
        <button class="btn-ghost" data-action="duplicate-action" data-choice="cancel" style="width: 100%;">
          Cancel Import
        </button>
      </div>
    </div>
...
- CRITICAL: Return ONLY the raw JSON object with no markdown formatting, no backticks, no explanatory text`;
  ...
  try {
    const _u = window._auth?.currentUser;
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Replaces the former inline on*= handler attributes, which the production
   CSP blocks (script-src-attr 'none') — including ones injected via
   innerHTML. One delegated listener per event type; elements declare their
   handler with data-action (click) / data-input (input).
   ═══════════════════════════════════════════════════════════════════════════ */

document.addEventListener('click', (e) => {
  const el = e.target.closest('[data-action]');
  if (!el) return;
  const num = el.dataset.num !== undefined ? Number(el.dataset.num) : undefined;
  switch (el.dataset.action) {
    case 'nav':
      e.preventDefault();
      navTo(el.dataset.nav || el.dataset.page);
      break;
    case 'manual-save': manualSave(); break;
    case 'start-next-session': startNextSession(); break;
    case 'view-session': viewSession(num); break;
    case 'view-session-detail': viewSessionDetail(num); break;
    case 'close-session-detail': closeSessionDetail(); break;
    case 'analyze-sessions': analyzeSessionsWithAI(); break;
    case 'generate-session-starter': generateSessionStarter(); break;
    case 'copy-session-starter': copySessionStarter(); break;
    case 'import-session-payload': importSessionPayload(); break;
    case 'manual-save-firestore': manualSaveToFirestore(); break;
    case 'parse-session-test': parseSessionTest(); break;
    case 'parse-all-sessions': parseAllSessions(); break;
    case 'duplicate-action':
      if (window.handleDuplicateAction) window.handleDuplicateAction(el.dataset.choice, num);
      break;
  }
});
...
document.addEventListener('input', (e) => {
  ...
  if (!el) return;
  if (el.dataset.input === 'filter-sessions') filterSessions(el.value);
  else if (el.dataset.input === 'execute-search') executeSearch();
});
