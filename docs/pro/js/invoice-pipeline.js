...
(function() {
  ...
  // the invoice unsent. When the outbox later sends it, it fires
  // 'nbd:sms-outbox-sent' with the source/sourceRef given here, and the
  // invoice is marked sent then — and only if it is still unsent.
  ...
  if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
    window.addEventListener('nbd:sms-outbox-sent', (ev) => {
      ...
      if (!d || d.source !== INVOICE_SMS_SOURCE || typeof d.sourceRef !== 'string' || !d.sourceRef) return;
      markInvoiceSentAfterQueuedSms(d.sourceRef)
        .catch((e) => console.warn('[invoice-pipeline] marking queued invoice text sent failed:', e && e.message));
    });
  }
    ...
    try {
      ...
      if (method === 'email') {
        ...
        if (window.NBDComms?.sendSMS) {
          ...
          // 'nbd:sms-outbox-sent' listener mark it sent when the text really
          // goes. Not awaited: offline, a Firestore write does not resolve
