...
function generate(prevLastmod, warn) {
  ...
  // Our Work project detail pages (docs/our-work/<slug>.html) — generated by
  // scripts/build-projects.mjs from docs/assets/data/projects.json, one flat
  // file per LIVE project; auto-discovered like blog/services/areas below so
  // a new project can never be "silently absent while the drift check says
  // zero diff" the way a hand-added top-level page would (see the note at
  // the top of this file about CORE_PAGES being curated, not globbed — this
  // section deliberately is NOT curated, for exactly that reason).
  const ourWorkSlugs = dropNoindexed(
  ...
  for (const slug of ourWorkSlugs) {
