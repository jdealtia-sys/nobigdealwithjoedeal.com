...
## ✅ V2-5 — CSP blocks the line-item formula evaluator → formula-qty items resolve to $0 — **FIXED (branch `feat/estimate-csp-formula-eval`)**
...
- **Fix shipped:** replaced `new Function()` (Layer 2 of `calcQuantity`) with `safeEvalFormula` — a CSP-safe recursive-descent evaluator over the SAME bounded grammar `validateFormula` already enforces (numeric literals · whitelisted measurement vars · the 8 Math helpers as bare `ceil(...)` or `Math.ceil(...)` · `+ - * / %` · unary `+ - !` · comparisons · `&& ||` · ternary · parens), with identical JS numeric/boolean semantics, purely over numbers/booleans — no object/string/property access. Layer 1 whitelist + `FORMULA_BLOCKED` cache unchanged.
- **Proof:** `tests/estimate-formula-eval.test.js` — DIFFERENTIAL test: 57 unique live-catalog + stressor formulas × 9 measurement contexts = 513 comparisons, each asserting the new evaluator returns the EXACT number the old `new Function` path computed; plus hand-computed value pins and sandbox-escape rejection checks. All green. Wired into `npm test` as `test:formulaeval`.
