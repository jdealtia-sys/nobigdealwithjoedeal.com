...
### ✅ 3B — V2 estimate reopen path — **SHIPPED** (branch `feat/estimate-v2-reopen`)
Saved V2 estimates now reopen into the **V2 builder** (not Classic): `viewEstimate` routes `builder:'v2'` docs to `openEstimateV2Builder({estimateId})` (and never falls through to Classic — that would mis-convert the doc, PR #507 class). `rehydrateFromSaved` repopulates customer + measurements (full reset, no bleed) + scope (catalog codes from rows) + pass-through fees + mode/tier/county, and stashes the raw doc. `finalize()`/`save()` use `effectiveEstimate()`: on a **clean** reopen of a 3A-capable doc it REPLAYS the saved numbers via `_reconstructEstimateFromSaved` (honoring persisted `materialMarkupPct` → B-8 retail reconciles); pre-3A docs or any edit fall back to a live re-resolve. `_editingEstimateId` makes a re-save UPDATE the same doc; `close()` clears it so it can't bleed into Classic. A 3-skeptic adversarial review found 4 blocking issues — all fixed: pass-through-fee loss on edit-after-reopen, Internal-View Hard Cost $0, stale measurement DOM inputs, and the Classic-fall-through corruption guard. Tests: `estimate-v2-payload.test.js` (44 assertions). **Live-verify post-deploy:** end-to-end reopen → edit → save (fee survives), per-SQ reopen, multi-reopen bleed, Internal-View Hard Cost.

### ✅ 3A — persist `materialMarkupPct` through save — **SHIPPED**; 3B (V2 reopen) now SHIPPED too (above)
...
### ✅ Drift cleanups (D-1/D-2/D-4) — **SHIPPED** (Jo-approved "unify Classic onto V2/config", 2026-06-09)
The deprecated Classic engine now produces the validated V2/config values:
- **D-2 (waste-from-pitch):** `estimates.js recommendedWasteForPitch` delegates to `EstimateBuilderV2.wasteFactorForPitch`, converting Classic's slope-area factor to rise/run (`√(factor²−1)`); legacy table is the fallback. Verified the standard pitches map onto V2's exact buckets (`estimate-engine-parity.test.js`).
- **D-4 (extra pipe boot):** unified at **$85** in `estimate-config.js` (`ADDON_EXTRA_PIPE_BOOT`); Classic reads it instead of the legacy `$45` (`window.R.pipe`). The product-library *install* rate (`R.pipe` in `getInternalCostBasis`) is a different quantity and correctly untouched.
- **D-1 (permit):** Classic's per-city defaults realigned to each city's **county** V2 value (Cincinnati→Hamilton $185, Mason→Warren $165, Milford→Clermont $170, KY cities, etc.); documented mapping; permit stays rep-overridable.
- **D-3 (tax key shape):** left as-is — each engine is internally self-consistent (V2 slugs, Classic county names); only a cross-engine latent risk, no active mismatch.
- Plus **FU-1:** V2 `save()` now persists insurance **claim** info (`est.claim`, separate from Classic's `est.insurance`) + `rehydrateFromSaved` restores it, so a reopened insurance estimate shows its claim header. Firestore-safe (null when empty, `num()` guards). Tests in `estimate-v2-payload.test.js`.
Cleared by a 2-skeptic adversarial review (GO; V2 calc path provably untouched). Live-verify post-deploy: a Classic estimate's waste/pipe/permit deltas + a reopened V2 estimate's claim header.
