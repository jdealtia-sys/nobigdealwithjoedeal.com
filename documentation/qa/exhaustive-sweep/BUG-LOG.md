...
**NEW-D30 — P1 cluster — /pro/vault.html data layer is entirely dead, four ways — FIXED in code (verified 2026-09-02): `vault-auth.module.js:25-27` sets `window._firestore` and dispatches `firebase-ready`; `firestore.rules:1309/1313` carry `codex/` + `codex-sessions/` blocks; `vault.html` has zero inline `oninput=`. The keep-or-kill product question no longer gates anything.**
...
**NEW-D31 — P3 — photo-review bulk Share: the portal-link copy half always throws, and its error toast is invisible — FIXED 2026-09-02**
...
*2026-09-02 update:* the toast half had already been closed (photo-review.html loads `toast.js` and `photo-review.js` bridges `window.showToast`), which turned the silent failure into a **visible** one — every share showed "Shared N photos" then a red "Couldn't copy link: Portal module not loaded". Neither fix choice above was taken: `state.lead.portalUrl` is no longer a thing (the helper deliberately mints fresh revocable tokens, and persisted portal URLs were purged in the 2026-08-18 storage-token work), and loading customer-portal.js on this page would drag in the whole customer surface. Instead `pages/photo-review.js` now defines `window._mintPortalUrl` with the same `createPortalToken` contract the dashboard uses (`dashboard-api.js`), which is the second minter `PortalLinkHelpers._minter()` already looks for. Pinned in `tests/smoke/photo.test.js`.
...
### New findings (d8 maps/draw tail, 2026-06-10-C)

Drove all of #/map (rows 009–034) and #/draw (035–075) live with REAL computer-tool clicks (synthetic .click() doesn't trip Leaflet/map-click handlers). Window kept foregrounded throughout (Leaflet/rAF pause when occluded). All ZZ_QA test data deleted; see CLEANUP.md. Seven new bugs (NEW-D34..D40); the rest PASS.

**NEW-D34 — LOW — Map FAB active-highlight desyncs from the sidebar layer toggles (dead wrapper)**
`dashboard-ui.js:2352` wraps `window.toggleOverlay` to mirror the toggled layer's state onto the matching `#fab-<type>` button. But `maps-core.js` defines `toggleOverlay` and loads AFTER dashboard-ui in dashboard.html, so the wrapper is clobbered — `window.toggleOverlay.toString()` no longer contains `_origToggleOverlay`. Net effect: toggling a layer from the SIDEBAR leaves the bottom FAB-bar highlight stale (and vice-versa is fine because `fabToggle` calls `toggleOverlay` then syncs its own class inline). Both directions still TOGGLE the layer correctly — only the FAB highlight is wrong. Fix: re-apply the FAB-sync wrapper after maps-core loads, or fold the FAB-class update into the canonical `toggleOverlay`. *Rows:* d8-tools-029 (PASS w/ caveat).

**NEW-D35 — MEDIUM — Jobs Overlay is permanently EMPTY for migrated tenants (legacy stage-name filter) — FIXED: PR #642 (2026-06-11)**
`buildJobsLayer()` (maps-overlays.js:35) filters active jobs with `['In Progress','Complete','Finalizing'].includes(l.stage)` — the legacy DISPLAY names. Post-crm-stages migration the canonical stage ids are `install_in_progress` / `closed` / etc., so the filter matches nothing and the Jobs layer shows zero markers even when the tenant has live jobs (JD has install_in_progress + closed leads, 0 rendered). Ironically the colorizer 20 lines below already handles BOTH name sets — the v159.4 stage sweep fixed the colors but missed the filter. Fix: include canonical ids in the filter (mirror the colorizer's `_stg` comparison). *Row:* d8-tools-013 (FAIL).

**NEW-D36 — MEDIUM — Spyglass search panel renders UNDER the page header → input + both buttons unclickable — FIXED: PR #642 (desktop `.map-view` is now position:relative so the panel anchors inside the map view; mobile overrides unaffected)**
`#mapSpyglassPanel` is `position:absolute; top:10px` but its offsetParent is BODY (it sits OUTSIDE the `position:relative .map-area`), so it overlaps the page header band. `elementFromPoint` at the panel's center hits `<HEADER>`, not the input — a user cannot type an address or click the Go / my-location buttons. `spyglassSearch()` works when invoked programmatically (geocoded Versailles z18), so the geocode+intel logic is fine; the fix is purely positional (move the panel inside `.map-area` or raise it above the header / drop its top offset). *Rows:* d8-tools-026/027/028 (all FAIL).

**NEW-D37 — MEDIUM — Zone-draw clicks ALSO fire the pin-confirm dialog (unconditional map-click handler) → stray-pin trap — FIXED: PR #642 (`if(zoneDrawing) return;` guard)**
`maps-core.js:99` binds `mainMap.on('click', e => openPinConfirm(...))` unconditionally, and `startZoneDraw` adds a SECOND click handler rather than suppressing the first. So while drawing a territory zone, every click both adds a zone vertex AND pops the Confirm-Pin-Placement overlay. During QA a zone-vertex click that landed on the overlay's "Save Pin" button SILENTLY SAVED A STRAY PIN to Firestore (deleted via deletePin). Real users drawing a multi-point zone get the pin dialog flashing on every click and can accidentally drop pins. Fix: guard the map-click pin handler with `if(zoneDrawing) return;` (and the analogous draw-mode guards). *Row:* d8-tools-020 (PASS w/ caveat — the zone draw itself works).

**NEW-D38 — LOW — Re-entering Draw-New-Zone stacks orphaned click handlers → each click adds multiple points — FIXED: PR #642 (off() the previous `_zoneClick` before reassigning; #642 also fixed the zone-list ✕ delete no-op — renderZoneList passed a string dataset id into deleteZone's numeric strict-compare)**
`startZoneDraw` assigns `mainMap._zoneClick = ...; mainMap.on('click', mainMap._zoneClick)` every time it runs, but `cancelZoneDraw`/`saveZone` only `off()` the LATEST handler. Entering zone-draw a 2nd time without a full view remount leaves the prior handler attached, so a single map click registers 2+ zone points (observed pts jumping 0→2 on one click after a re-entry). Cosmetic-ish (the polygon is still drawable) but compounds NEW-D37. Fix: `off` the specific handler in cancel/save (already stored on `_zoneClick`) — it currently does, but re-entry overwrites the reference before the old one is removed; null-check + remove-before-reassign. *Row:* d8-tools-020 note.

**NEW-D39 — MEDIUM — "Generate Estimate" from Drawing: imported measurements are SILENTLY overridden when a V2 draft exists — FIXED: PR #643 (measurements ride through `openEstimateV2Builder(opts).importMeasurements` — the lazy stub forwards args — and apply AFTER draft restore / lead prefill; builder owns the toast, incl. an explicit draft-replaced variant)**
`importToEstimate()` opens the V2 builder then `setTimeout(300)`-sets v2rawSqft/v2eaveLf/etc. But `openEstimateV2Builder()` restores `localStorage.nbd_v2_draft_v1`, and that restore wins over the import: first live run the dialog said "Pitched area: 7776 SF" yet the builder showed the stale draft's **200 SF** and customer "ZZ_QA_ ServerPDF Test" — while the "Drawing measurements imported into V2 Builder" toast still fired (false success). Proven by re-test with the draft deleted: import then correctly landed 6000 SF / 120 eave / 80 ridge. Compounding: the V2 builder autosaves a draft on ANY input (my import re-created one), and customer fields are NOT reset on modal close, so a draft almost always exists in practice → the import is routinely discarded. Fix: have importToEstimate clear/override the draft, or run the field-set AFTER (not racing) draft-restore, or skip draft-restore when an import is pending. *Row:* d8-tools-048 (PASS — core import works with no draft; this is the override defect).

**NEW-D40 — bundle of two smaller map defects — (a) FIXED: PR #645; (b) OPEN**
(a) **MEDIUM — "Load from Customer" is rules-denied. — FIXED: PR #645.** Root-cause correction: `leads/{id}/drawings` has had owner rules since PR #38 — the live denial came from the UNLINKED fallback branch (top-level `/drawings`, used when no lead matches the address), which had no block at all. #645 adds the owner-scoped top-level rule, emulator regression tests for both client query shapes (rules case 25, + cross-tenant suite green), and registers `drawings` in the GDPR `FLAT_USER_COLLECTIONS` registry (the registry-drift smoke guard caught the omission). _Original entry:_ `loadDrawingFromCustomer()` queries `leads/{id}/drawings` (or top-level `drawings` for unlinked) ordered by version; live result is `FirebaseError: Missing or insufficient permissions` — the `drawings` subcollection has no read rule for the owner, so saved roof drawings can never be loaded back. Mirror of NEW-D8/D14 (a feature whose Firestore path is default-denied). Save path likely writes fine but is unverifiable end-to-end without a matching read rule + test. *Row:* d8-tools-050.
(b) **LOW — Pins drawn this session don't heat-map / "Clear All Pins" is display-only with no confirm.** `commitPin`→`refreshHeatLayer`→`buildJobsLayer/buildHeatLayer` read `window._pins`, but `_savePin` never appends the new pin to that array, so a just-dropped pin isn't in the heat layer until a reload. Separately, `clearAllPins()` wipes markers from the map (`pinMarkers={}`) with NO confirm despite the row's `danger:delete` — but it does NOT delete the Firestore pin docs (display-only; markers reappear on view remount). Net: low-risk, but the "Clear All Pins" label oversells a destructive-looking action that's actually cosmetic, and new pins silently miss the heat layer. *Rows:* d8-tools-011 / 025 notes.

### Notes from the mobile pass (2026-06-10-C) — no new bugs, two things to record

**TOOLING LIMIT (record for future mobile passes):** the `resize_window` MCP tool could NOT shrink the maximized QA Chrome window — it returns success but `innerWidth` stays 1920, so the width-gated mobile CSS media queries never activate. The whole mobile pass therefore verified the delegate/handler LOGIC (force-show the mobile chrome via a CSS override + REAL clicks on the live delegates + direct handler invocation) but NOT the true narrow-viewport CSS reflow or real touch. A real device (or a Chrome instance that honors window resizing / CDP device emulation) is needed to close that gap for gap-mobile, d3-pipeline-011, d1-chrome-012/013, and d8-tools-031/072.

**INVENTORY DIVERGENCE (not a bug — `mobile-nav-customizer.js` feature):** the live mobile bottom nav is rebuilt at runtime by `mobile-nav-customizer.js renderBottomNav()` with `data-mnc-action`/`data-mnc-id` delegates and the user's CONFIGURED tab set (Jo's = Home / D2D / Photos / CRM / More). So the exhaustive-sweep inventory's static-markup selectors — `data-action="mobileNav"`, the center New-FAB `#mni-create` (`mCreateFabRoute`), and `#mni-joe` — DO NOT EXIST in the live DOM (the customizer replaced them with D2D + Photos). The underlying routing (`mobileNav`→`goTo`), the Create popover (`toggleMobileCreatePopover`/`_mCreate`), and Ask-Joe are all still reachable (joe via the More drawer), so nothing is broken — but any future re-inventory of the mobile bar should read the customizer output, not dashboard.html:12076. The customizer also overrides `window.mobileNav` to drive active-state off the custom tab list. Worth a one-line note in the inventory so the create-FAB/joe rows aren't mistaken for missing controls.

## 2026-06-10-C (cont.) — d7-settings detail pass (116 rows, workflow-assisted)

Static analysis (6 parallel repo-readers; appearance + team/billing/access/help analyzers and the whole adversarial-verify phase died on the monthly subagent spend limit) + live single-driver browser pass over Profile/global, Notifications, Company, Company Profile, Daily OS, and Estimates tabs. Every mutated setting snapshotted first and restored byte-exact (LS diffs verified; shop-wide companyProfile verified token-free after restore). Five new findings:

**NEW-D41 — MEDIUM — Five notification toggles persist but gate NOTHING at runtime — DEFERRED 2026-06-11 (product decision: wire consumers or remove the toggles; log-only per remediation directive)**
`#notifHot`, `#notifStorm`, `#notifInbound`, `#notifNeedsField` (triggers) and `#chEmail` (channel) all save + round-trip perfectly (verified live), but no code consumes them: storm alerts fire from `storm-integration.js triggerProactiveAlert` with no `shouldFireNotif` check; hot-lead actions are built by `ask-joe-proactive.js` without consulting the gate; `inbound_msg`/`needs_field` exist only in the type map (zero firing callers repo-wide); the email channel is mapped but no caller passes `'email'` and no Cloud Function reads `userSettings.notifications`. Only real consumers anywhere: `tasks.js` (task_overdue → notifOverdue, chInApp, chPush) and `review-engine.js:126` (estimate_approved → notifApproval, firehose-mode only). Users configuring these toggles get no behavior change. *Rows:* d7-settings-169/170/172/174/177 (FAIL).

**NEW-D42 — MEDIUM — The entire Company tab is store-only (no runtime consumer) — DEFERRED 2026-06-11 (product decision: consume in doc-gen or fold into Company Profile)**
All 18 `co*` fields save to LS `nbd_company_settings` + Firestore `userSettings.company` and round-trip flawlessly — but nothing in the codebase reads either store at runtime. Doc-gen letterhead/license/phone come from the separate Company Profile tab (`companyProfile/{companyId}`); `document-generator.js` has zero references to any `co*` field. A user filling in license numbers / insurance / GAF cert here reasonably expects them on documents; they go nowhere. Fix direction: either consume these fields (fallback into doc-gen) or fold the tab into Company Profile. *Rows:* d7-settings-063..083-ish (company group, PASS round-trip + tab-level finding).

**NEW-D43 — MEDIUM — Estimates tab: 3 ghost fields + write-only cloud mirror + stale addon copy — (a) FIXED: PR #644 (ghost trio removed; panel retitled "Cost & Tax Defaults", wired siblings kept); (b)/(c) OPEN**
(a) `#defPitch` / `#defWaste` / `#defDeckPct` are markup-only ghosts: zero JS references; Save All silently discards them and reload reverts to hardcoded HTML defaults (live-verified with tokens 124/125/126). (b) The Firestore `userSettings.estimateSettingsV2` mirror written by `_saveEstimateDefaultsV2` is never read back by anything — the claimed cross-device sync is one-way dead. (c) Save All updates `companyProfile.pricing.addonPrices` (shop-wide, calc-time override) but leaves `nbd_est_settings_v3.addonPrices` stale — two divergent copies of addon pricing. *Rows:* d7-settings ghost trio (FAIL), rest PASS.

**NEW-D44 — LOW (documented) — Profile tab ships four dead placeholder fields — DEFERRED 2026-06-11 (wire or remove; product decision — Company overlaps Company Profile)**
`#settingsCompany` / `#settingsPhone` / `#settingsRole` / `#settingsLicense` are acknowledged dead by the in-code comment at `dashboard-bootstrap.module.js:2931-2937` (not read by `_saveSettings`, never loaded). They render as normal editable inputs and silently discard input. Either wire them or remove them. *Rows:* d7-settings-014/015/016/017 (FAIL).

**NEW-D45 — LOW bundle — four cosmetic/UX strays — (a)(b)(c) FIXED: PR #646 (href kept in step; clearJoeKey resets the status line; kanban priming extracted to `_syncKanbanPrefControls()` and re-run from `switchSettingsTab('profile')` after template hydration); (d) OPEN (needs the swatch-key/theme-precedence design decision)**
(a) cal.com preview is an `<a>` whose `href` is never set (`dashboard-ui-prefs-boot.js:272-275` writes textContent only) — a dead link that looks live (d7-settings-019 FAIL). (b) `clearJoeKey` clears the key but leaves the stale "✓ Key saved — Joe AI is active" status line (d7-settings-032 FAIL). (c) Kanban density active-state repaint after reload is dead (`dashboard-ui.js:1589-1601` DCL+200ms painter; the saved density itself applies fine — html[data-density] correct; the bold/auto-collapse checkbox priming suspicion did NOT reproduce live on direct loads). (d) Daily-OS Program Theme round-trips but its consumers read `ds-theme` only as last fallback behind the dashboard theme keys, and the daily-success lookup lacks 8 of 12 swatch keys (static) — picked theme rarely visibly applies.

**Carry-note:** the 2 unanalyzed groups (appearance 25 rows, team/access/billing/help 16 rows) remain UNTESTED — the workflow analyzers for them hit the monthly subagent spend limit; the per-field method + this session's tab-cycle pattern apply directly next session.
