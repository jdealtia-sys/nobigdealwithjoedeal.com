...
**NEW-D5 — HIGH (CONFIRMED, 2026-06-09-B session) — ✅ FIXED (PR `fix/product-library-csp-delegates`) — Product Library view is 100% dead chrome: every button, filter, and the search box do nothing**
The `#/products` view (rendered by `docs/pro/js/product-library.js` into `#productLibraryContainer`) renders fine but **no control works**: + Add Product, Export CSV, Reset, tier chips, category pills, category collapse headers, and per-card Edit/Archive are all inert; the search box doesn't filter; the edit-modal Save/Delete/Cancel/× and the live margin recalc are dead. Two independent causes, same family as NEW-1:
1. All buttons carry `data-pl-action="…"` attributes, but **no `data-pl-action` click delegate exists on the dashboard** — the only one in the codebase is in `landing-page.js` (a naming collision; it handles its own `goRegister`/`toggleFAQ` and isn't loaded on dashboard anyway).
2. The search input and the 3 modal pricing inputs used inline `oninput="window._productLib…"` handlers, CSP-blocked on `/pro/dashboard` (no `unsafe-inline` in `script-src`). The per-card `onmouseenter`/`onmouseleave` hover lift was CSP-dead too.
*Repro:* dashboard → `#/products` → click any chrome (tier chip, category pill, + Add Product) → nothing happens; type in search → grid doesn't filter.
*Fix (this PR):* self-contained document-level delegates inside `product-library.js` — `data-pl-action` click delegate guarded by `typeof window._productLib[action] === 'function'` (collision-safe with landing-page.js in both directions), `data-pl-input` delegate for search (with focus/caret restore across the re-render) + modal margin recalc, `data-pl-hover` mouseover/mouseout for the card lift; `?v=3` cache bump in `script-loader.js` + `dashboard.legacy.html`.
