...
**NEW-D35 — MEDIUM — Jobs Overlay is permanently EMPTY for migrated tenants (legacy stage-name filter)**
...
**NEW-D36 — MEDIUM — Spyglass search panel renders UNDER the page header → input + both buttons unclickable**
...
**NEW-D37 — MEDIUM — Zone-draw clicks ALSO fire the pin-confirm dialog (unconditional map-click handler) → stray-pin trap**
...
**NEW-D38 — LOW — Re-entering Draw-New-Zone stacks orphaned click handlers → each click adds multiple points**
...
**NEW-D39 — MEDIUM — "Generate Estimate" from Drawing: imported measurements are SILENTLY overridden when a V2 draft exists**
...
**NEW-D41 — MEDIUM — Five notification toggles persist but gate NOTHING at runtime**
...
**NEW-D42 — MEDIUM — The entire Company tab is store-only (no runtime consumer)**
...
**NEW-D43 — MEDIUM — Estimates tab: 3 ghost fields + write-only cloud mirror + stale addon copy**
...
**NEW-D44 — LOW (documented) — Profile tab ships four dead placeholder fields**
...
**NEW-D45 — LOW bundle — four cosmetic/UX strays**
