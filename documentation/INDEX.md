...
- Current handoff: [NEXT_SESSION-2026-08-19](projects/NEXT_SESSION-2026-08-19.md)
...
- [JOB-TEMPLATE-COST-LEAK-2026-08-18](audit/JOB-TEMPLATE-COST-LEAK-2026-08-18.md) — `docs/pro/js/job-templates-data.js` publishes 84 contractor cost pairs (146 non-zero values) and `estimate-logic-engine.js` publishes the markup math beside it, so cost basis AND margin are both derivable; `catalog-cost-privacy.test.js` passes only because the file is on neither its STRICT_FILES nor exception list. Second allowlist-shaped guard failure of the day. Includes git-history assessment (one commit, 235 to rewrite, 10 worktrees — rotate the numbers instead)
